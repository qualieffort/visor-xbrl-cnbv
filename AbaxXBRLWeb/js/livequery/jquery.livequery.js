...
(function (factory) {
    ...
    $.extend($.fn, {
        ...
        expire: function (selector, matchedFn, unmatchedFn) {
            ...
            $.livequery.deleteQuery(this, selector, matchedFn, unmatchedFn)
            return this;
        }
    });
    ...
    $.extend($.livequery, {
        ...
        find: function (jq, selector, matchedFn, unmatchedFn) {
            var q;
            $.each($.livequery.queries, function (i, query) {
                if (selector === query.selector && jq.selector === query.jq.selector &&
        ...
        deleteQuery: function (jq, selector, matchedFn, unmatchedFn) {

            if ($.livequery.queries && $.livequery.queries.length) {
                for (var i = 0; i < $.livequery.queries.length; i++) {
                    var query = $.livequery.queries[i];
                    if (selector === query.selector && jq.selector === query.jq.selector &&
                           (!matchedFn || matchedFn.$lqguid === query.matchedFn.$lqguid) &&
                           (!unmatchedFn || unmatchedFn.$lqguid === query.unmatchedFn.$lqguid)) {
                        $.livequery.queries.splice(i, 1);
                        return;
                    }
                }
            }
