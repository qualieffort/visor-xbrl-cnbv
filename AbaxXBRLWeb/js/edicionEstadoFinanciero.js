...
$(document).ready(function () {
    ...
    $('#tablaEstadoInformacionGeneral').filterTable({
        inputSelector: '#filtroTabla'
    });

    $('#tablaEstadoResultado').filterTable({
        inputSelector: '#filtroTabla'
    });

    $('#tablaOtrosResultadosIntegrales').filterTable({
        inputSelector: '#filtroTabla'
    });

    $('#tablaDatosInformativosEstadoResultados').filterTable({
        inputSelector: '#filtroTabla'
    });

    $('#tablaDatosInformativosDoceMeses').filterTable({
        inputSelector: '#filtroTabla'
    });

    $('#tablaEstadoFlujosEfectivo').filterTable({
        inputSelector: '#filtroTabla'
    });
    ...
    $('.calendar').datepicker().on('changeDate', function (e) {
        if ($(this)[0].valorAnterior !== undefined && $(this)[0].valorAnterior != $(this).val() && $(this)[0].id && $(this)[0].id.startsWith('input-')) {
            validarFormatos();
            indicarCambiosPendientesGuardar();
        }
    });
    $('.acombo').each(function () {
        var valorActual = $(this).val();
        var miId = $(this)[0].id;
        $(this).replaceWith('<select id="' + miId + '" name="' + miId + '" class="form-control m-b acombo"><option value="SI">SI</option><option value="NO">NO</option></select>');
        $('#' + miId).val(valorActual);
    });

    $('.form-control').focus(function () {
        $(this)[0].valorAnterior = $(this).val();
    });

    $('.form-control').blur(function () {
        if ($(this)[0].valorAnterior != $(this).val() && $(this)[0].id  && $(this)[0].id.startsWith('input-')) {
            validarFormatos();
            indicarCambiosPendientesGuardar();
        }
    });
    ...
    if (detalleOperacion) {
        ...
        $('#btnActualizarDatos').bind('click', function () {
            ...
            for (i = 0; i < detalleOperacion.conceptosOperandos.length; i++) {
                var operando = detalleOperacion.conceptosOperandos[i];
                var valorOperando = $("#input_" + operando.id).autoNumeric('get');
                operando.conceptoAsociado.autoNumeric('set', valorOperando);
            }
            detalleOperacion.conceptoResultado.autoNumeric('set', $('#input_v_' + idElemento).autoNumeric('get'));
            validarFormatos();
...
function presentarDetalleDeCalculo(idContexto, idElemento) {
...
function agregarErrorAListado(detalleError) {
    ...
    detalleError.detalleOperacion.conceptoResultado.addClass("parsley-validated parsley-error");
    ...
    var idBtn = idConceptoResultado.replace("input", "btn");

    $('#' + idBtn + " i").removeClass('fa');
    $('#' + idBtn + " i").removeClass('fa-dollar');
    $('#' + idBtn + " i").removeClass('fa-calculator');
    $('#' + idBtn + " i").removeClass('fa-font');
    $('#' + idBtn + " i").removeClass('fa-fa-sort-numeric-asc');
    $('#' + idBtn + " i").removeClass('fa-fa-list-ul');

    if (detalleError.detalleOperacion.conceptosOperandos.length == 0) {
        $('#' + idBtn + " i").addClass('fa');
        $('#' + idBtn + " i").addClass(detalleError.detalleOperacion.concepto.ICONO);
    } else {
        $('#' + idBtn + " i").addClass('fa');
        $('#' + idBtn + " i").addClass('fa-calculator');
    }

    $('#' + idBtn + " i").addClass('text-danger');
    var idA = idBtn.replace("btn-", "a-");
...
function validarEstadoFinanciero(validaciones, idContexto) {
    ...
    for (iVal = 0; iVal < validaciones.length; ++iVal) {
        ...
        if (!detalleErrores["input-" + idContexto + "-" + cuentaEmisnetResultado]) {
            ctaResultado.removeClass('parsley-validated');
            ctaResultado.removeClass('parsley-error');
            ctaResultado.tooltip('destroy');

            $(idBoton + " i").removeClass('fa');
            $(idBoton + " i").removeClass('fa-dollar');
            $(idBoton + " i").removeClass('fa-calculator');
            $(idBoton + " i").removeClass('text-danger');
            $(idBoton + " i").removeClass('text-default');
            $(idBoton + " i").removeClass('text-success');

            $(idBoton + " i").addClass('fa');
            $(idBoton + " i").addClass(definicionConceptoResultado.ICONO);
            $(idBoton + " i").addClass('text-default');
        }

        var valCtaResultado = '';

        if (ctaResultado.hasClass('acurrency') || ctaResultado.hasClass('anumeric')) {
            valCtaResultado = ctaResultado.autoNumeric('get');
        } else {
            valCtaResultado = ctaResultado.val();
        }
        ...
        if (valCtaResultado === undefined || valCtaResultado === "") {
            ...
            for (iOper = 0; iOper < validacion.operadores.length; iOper++) {
                ...
                if (detalleOperando.conceptoAsociado.hasClass('acurrency') || detalleOperando.conceptoAsociado.hasClass('anumeric')) {
                    valorOperando = detalleOperando.conceptoAsociado.autoNumeric('get');
                } else {
                    valorOperando = detalleOperando.conceptoAsociado.val();
                }
            ...
            if (detalleOperacion.valorCalculado != detalleOperacion.valor) {
                ...
                for (iOper = 0; iOper < validacion.operadores.length; iOper++) {
                    ...
                    $('#' + idBtn + " i").removeClass('text-danger');
                    $("#input-" + idContextoOperador + "-" + cuentaEmisnet).removeClass('parsley-validated');
                    $("#input-" + idContextoOperador + "-" + cuentaEmisnet).removeClass('parsley-error');
                }
                ...
                if (!detalleErrores["input-" + idContexto + "-" + cuentaEmisnetResultado]) {
                    ...
                    $(idBoton + " i").addClass('fa');
                    $(idBoton + " i").addClass('fa-calculator');
                    $(idBoton + " i").addClass('text-success')
                }
            }
    ...
    for (i = 0; i < listadoVersiones.length; i++) {
        ...
        if (esUltimaVersion) {
            ...
            $('#versionesTimeLine').append('<article class="timeline-item ' + (izquierda ? 'alt' : '') + '">' +
                                            ...
                                            '                   <a  data-bjax="true" href="/DocumentosInstancia/EditorDocumentoInstanciaXbrl?idDocumentoInstancia=' + version.IdDocumentoInstancia + '&numeroVersion=' + version.Version + '" class="btn btn-sm btn-default btn-bg"><i class="fa fa-reply"></i>&nbsp;&nbsp;&nbsp;&nbsp;Cargar Datos</a> ' +
                                            '               </div> ' +
                                            '           </div>' +
                                            '       </div>' +
                                            '   </div>' +
